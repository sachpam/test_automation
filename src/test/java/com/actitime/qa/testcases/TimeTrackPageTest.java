...
public class TimeTrackPageTest extends TestBase{
    ...
    public void approveTimeSheet() {
        timeTrackPage.clickApproveTimeTrackLink().timeTrackPageDisplays();
        timeTrackPage.clickReadyCheckbox().clickApproveReady();
    }

    @Test(priority = 4)
    public void rejectTimeSheet() {
        timeTrackPage.clickApproveTimeTrackLink().timeTrackPageDisplays();
        timeTrackPage.clickReadyCheckbox().clickRejectReady();
    }

    //for time sheets with not-ready status
    @Test(priority = 5)
    public void approveTimeSheetNotReady() {
        timeTrackPage.clickApproveTimeTrackLink().timeTrackPageDisplays();
        timeTrackPage.clickNotReadyCheckbox().clickApproveNotReady();
    }

    @Test(priority = 6)
    public void rejectTimeSheetNotReady() {
        timeTrackPage.clickApproveTimeTrackLink().timeTrackPageDisplays();
        timeTrackPage.clickNotReadyCheckbox().clickRejectNotReady();
    }
